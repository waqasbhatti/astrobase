    ...
    def get(self, checkplotfname):
        ...
        if checkplotfname:
            ...
            if self.checkplotfname in self.currentproject['checkplots']:
                ...
                # - fitinfo
                # this contains the following dict
                # fitinfo[fitmethod] = {'fitchisq','fitredchisq','fitepoch',
                #                       'fitphase','fitmags','fitplot'}
                # the fitplot is generated by the function
                # checkplot._pkl_phased_magseries_plot with the overplotfit
                # kwarg set to the results of a varbase.lcfit function. Only the
                # fitpoech, fitchisq, fitredchisq, and fitplot are passed to the
                # frontend.
                if 'fitinfo' in cpdict and isinstance(cpdict['fitinfo'], dict):
                    fitinfo = cpdict['fitinfo']
                    if isinstance(fitinfo['fitplot'], bytes):
                        fitinfo['fitplot'] = fitinfo['fitplot'].decode()
                else:
                    fitinfo = None
